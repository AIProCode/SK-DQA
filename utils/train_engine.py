...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            patch_feat_iter = patch_feat_iter.cuda()
            cap_feat_iter = cap_feat_iter.cuda()
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                sub_patch_feat_iter = \
                    patch_feat_iter[accu_step * __C.SUB_BATCH_SIZE:
                                (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_cap_feat_iter = \
                    cap_feat_iter[accu_step * __C.SUB_BATCH_SIZE:
                                (accu_step + 1) * __C.SUB_BATCH_SIZE]
