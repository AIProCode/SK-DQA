...
from transformers import BertTokenizer, DistilBertTokenizer, MobileBertTokenizer
...
class DataSet(BaseDataSet):
    def __init__(self, __C):
        ...
        # patch_feat_path_list = \
        #     glob.glob(__C.FEATS_PATH[__C.DATASET + '_patch']['train'] + '/*.npy') + \
        #     glob.glob(__C.FEATS_PATH[__C.DATASET + '_patch']['val'] + '/*.npy') + \
        #     glob.glob(__C.FEATS_PATH[__C.DATASET + '_patch']['test'] + '/*.npy')
        # cap_feat_path_list = \
        #     glob.glob(__C.FEATS_PATH[__C.DATASET + '_cap']['train'] + '/*.npy') + \
        #     glob.glob(__C.FEATS_PATH[__C.DATASET + '_cap']['val'] + '/*.npy') + \
        #     glob.glob(__C.FEATS_PATH[__C.DATASET + '_cap']['test'] + '/*.npy')
    ...
    def img_feat_path_load(self, path_list):
        ...
        for ix, path in enumerate(path_list):
            iid = str(int(path.split('/')[-1].split('.')[0]))
            # print(iid)
    ...
    def load_img_feats(self, idx, iid):
        frcn_feat_iter = np.load(self.iid_to_frcn_feat_path[iid])
